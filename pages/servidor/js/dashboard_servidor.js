document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarHistoricoSolicitacoes() {
        try {
            const response = await fetch('./functions/get_historico_solicitacoes_servidor.php');
            ...
            if (data.sucesso && tabelaHistorico) {
                ...
                if (data.solicitacoes.length === 0) {
                    tabelaHistorico.innerHTML = '<tr><td colspan="6" style="text-align:center;">Nenhum registro encontrado.</td></tr>';
                } else {
                    data.solicitacoes.forEach(s => {
                        ...
                        row.innerHTML = `
                            ...
                            <td data-label="Tipo">${s.colorida == 1 ? 'Colorida' : 'P&B'}</td>
    ...
    async function carregarCotas() {
        try {
            const response = await fetch('./functions/get_cota_servidor.php');
            const data = await response.json();
            if (data.sucesso && data.cota) {
                const c = data.cota;
                const pbDisp = parseInt(c.cota_pb_total, 10) - parseInt(c.cota_pb_usada, 10);
                const colorDisp = parseInt(c.cota_color_total, 10) - parseInt(c.cota_color_usada, 10);
                if (cotaInfoEl) {
                    cotaInfoEl.innerHTML = `Cota P&B: <strong>${pbDisp}</strong> | Cota Colorida: <strong>${colorDisp}</strong>`;
                }
            } else {
    ...
    async function carregarSolicitacoes() {
        try {
            const response = await fetch('./functions/get_solicitacoes_servidor.php');
            const data = await response.json();
            const container = document.getElementById('tabela-solicitacoes');
            if (data.sucesso && container) {
                ...
                if (data.solicitacoes.length === 0) {
                    html += '<tr><td colspan="6" style="text-align:center;">Nenhuma solicitação recente.</td></tr>';
                } else {
                    data.solicitacoes.forEach(s => {
                        const dataFormatada = new Date(s.data_criacao).toLocaleString('pt-BR');
                        const arquivoHtml = s.arquivo_path
    ...
    if (formSolicitacao) {
        ...
        formSolicitacao.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('./functions/processar_solicitacao_servidor.php', {
                    method: 'POST',
                    body: formData
                });
                const data = await response.json();
