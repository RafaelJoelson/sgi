...
try {
    // 1. Busca por notificações não visualizadas para o usuário logado
    $stmt = $conn->prepare("SELECT id, mensagem FROM Notificacao WHERE destinatario_cpf = :cpf AND visualizada = FALSE");
    $stmt->execute([':cpf' => $cpf_usuario]);
    $notificacoes = $stmt->fetchAll(PDO::FETCH_ASSOC);

    if ($notificacoes) {
        // 2. Marca as notificações como visualizadas para não mostrá-las novamente
        ...
        // 3. Retorna as notificações para o frontend
